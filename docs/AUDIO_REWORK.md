...
- Build-identity follow-up: CMake embeds the local Git revision (with `-dirty` when appropriate), configuration UTC, application version and JUCE pin. Source folders without their own `.git` are labelled `source-archive`, not an enclosing repository's revision. Diagnostics and the title tooltip show identity; `--build-info` exports JSON before constructing/enumerating the audio engine. Portable packaging collects that JSON from the staged executable and adds its independently computed SHA256. Packaging also rejects a modified JUCE checkout rather than archiving a different dependency tree. Local JUCE HEAD was checked against the pin and its worktree was clean.
- Identity verification and packaging passed (`build/evidence-20260908-215719`, initial package `dist/CantoDeck-0.1.0-win-x64-20260908-215728.zip`): JSON fields and 64-bit architecture validated, diagnostic identity matched, and the staged executable hash matched its manifest. This initial build correctly reports a dirty checkpoint, not a clean final commit. CTest passed 3/3; offline stream/record/fault/UI tests also passed. Fresh device enumeration still showed Realtek/Steam endpoints and no DGM20; no physical loop measurement was attempted.
- Hosted Windows CI passed for the streaming checkpoint `bdf2078`: [run 34239934025](https://github.com/tang-vu/cantodeck/actions/runs/34239934025). Later monitoring-policy and build-identity changes are not covered by that hosted result.
- Compatibility-monitor fix: pinned JUCE WASAPI `stop()` synchronously invokes `audioDeviceStopped()` even for our short control pauses. That previously disabled monitoring when opening a track. A thread-local, engine-specific scope now exempts only this deliberate output callback stop; input stops, other-thread/other-engine output stops, errors, close and device changes retain their mute behavior. Recording control no longer restores an old monitor flag after suspension, avoiding accidental re-enabling after a concurrent mute/error.
- Policy tests cover expected/unexpected input/output stops, cross-thread/cross-engine isolation and exception cleanup. Release/CTest passed 3/3 and full offline/UI verification passed (`build/evidence-20260908-215022`). An explicit `--test-compat-control` run passed on Realtek mic array -> Headphones endpoint at 48 kHz/480 frames (`build/compat-control-20260908-214956.txt`, exit 0). MUTE ALL was set before opening and never cleared; no recorder or tone was started. The test toggled only monitor intent, loaded a synthetic WAV, exercised recording-stop control with no active take, verified intent preservation/non-reenable, zero output peak and monitoring-off after close. This is not audible monitoring or actual unplug validation.
