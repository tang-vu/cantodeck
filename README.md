...
Signal path: input mapping → clock bridge → smoothed software input boost (0–24 dB, default 0) → vocal processing and mic gain → monitored voice plus stereo WAV → smoothed master → stereo-linked sample-peak limiter at 0.95. Transparent voice (default in the UI) bypasses coloration and effects while retaining gain. The processed path offers a 75 Hz high-pass, optional expander, broad tone EQ, compressor, echo and four damped room combs. Input meters and dry recording remain pre-boost. EQ is not fully parametric; the room effect is basic. No de-esser, true-peak limiter or automatic anti-feedback is included.
