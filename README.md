...
Track/device/record changes are disabled while opening a track; emergency mute remains available. Do not change tracks during recording. End of track stops; choose the next queue item explicitly. The local session persists up to 128 queued WAV paths, manual lyric offset and per-song LRC/TXT associations. Startup restores descriptors without opening/selecting/playing media; select a song explicitly, and missing files report a load error then. A successfully opened lyric file is associated with the currently loaded song. Without an association, same-name LRC lookup remains available. Clear queue removes list entries and saved associations, not files, displayed lyrics or the currently loaded song. Playlist/lyric paths and manual offset are excluded from exported vocal presets. MP3, FLAC and video are disabled.
